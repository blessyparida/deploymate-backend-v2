...
export function generateConfigs(repoDir: string, detected: Record<string, any>) {
  ...
  const writeFile = (filename: string, content: any) => {
    try {
      if (typeof content !== "string") {
        console.error(`⚠️ Invalid content for ${filename}:`, content);
  ...
  // 1️⃣ Dockerfile (Express, Flask, FastAPI)
...
`
  ...
  // 3️⃣ Netlify config (React or Vue)
...
`
  ...
  // 4️⃣ README summary — always generated
  writeFile(
